...
class StockController(
    ...
    fun getStockHistoryById(
        @PathVariable id: String,
        @RequestParam(defaultValue = "LIVE") timeframe: String
    ): ResponseEntity<*>{
        val result = stockService.getStockHistory(id.toLong(),timeFrameToEnum(timeframe))
        return dataResponse(
            status = HttpStatus.OK,
    ...
    private fun timeFrameToEnum(timeframe: String): TimeFrame {
        return when (timeframe.uppercase()) {
            "1M" -> TimeFrame.ONE_MINUTE
            "5M" -> TimeFrame.FIVE_MINUTES
            "30M" -> TimeFrame.THIRTY_MINUTES
            "ALL" -> TimeFrame.ALL
            else -> throw IllegalArgumentException("Invalid timeframe: $timeframe")
        }
    }
}
