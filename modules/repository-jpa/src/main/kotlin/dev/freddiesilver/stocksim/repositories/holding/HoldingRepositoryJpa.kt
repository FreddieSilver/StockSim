...
class HoldingRepositoryJpa(
    ...
    override fun findByUserAndStock(
        userId: Long,
        stockId: Long,
    ): Holding? = jpa.findByUserIdAndStockId(userId, stockId)?.let { HoldingMapper.toDomain(it) }

    override fun findByUserId(userId: Long): List<Holding> = jpa.findByUserId(userId).map { HoldingMapper.toDomain(it) }

    override fun findById(id: Long): Holding? = jpa.findById(id).orElse(null)?.let { HoldingMapper.toDomain(it) }

    override fun findAll(): List<Holding> = jpa.findAll().map { HoldingMapper.toDomain(it) }
